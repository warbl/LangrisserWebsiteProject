...
public class DbMods {
    ...
    public static StringData newFind (DbConn dbc, String minMembership, String maxBirthday) {
        ...
        if ((myEmail == null) || (myPassword == null) || myEmail.length()>45 || myPassword.length() > 45) {
            sd.errorMsg = "Error: Must provide a valid email and a valid password";
            return sd;
        }
        ...
        try {
            ...
            // Encode the id (that the user typed in) into the select statement, into the first (and only) ? 
            stmt.setString(1, myEmail); // 1 means the first  ? in the SQL
            stmt.setString(2, myPassword);          // 2 means the second ? in the SQL    
            ResultSet results = stmt.executeQuery();
            if (results.next()) { // id is unique, one or zero records expected in result set
    ...
    public String userRoleType = ""; // getting it from joined user_role table.
        ...
        errorMsgs.userPassword2 = ValidationUtils.stringValidationMsg(inputData.userPassword2, 45, true);
